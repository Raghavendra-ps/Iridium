...
def get_jobs_by_owner(db: Session, *, owner_id: int) -> list[ConversionJob]:
    ...
    """
    return db.query(ConversionJob).filter(ConversionJob.owner_id == owner_id).order_by(ConversionJob.created_at.desc()).all()
...
def get_job_by_id(db: Session, *, job_id: int, owner_id: int) -> ConversionJob | None:
    ...
    """
    return db.query(ConversionJob).filter(ConversionJob.id == job_id, ConversionJob.owner_id == owner_id).first()

def create_job(db: Session, *, owner_id: int, original_filename: str, storage_filename: str, target_doctype: str, target_org_id: int) -> ConversionJob:
